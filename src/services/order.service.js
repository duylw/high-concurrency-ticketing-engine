    ...
    const completedOrder = await prismaClient.$transaction(async (tx) => {
        ...
        for (let i = 1; i <= order.quantity; i++) {
            ...
            const qrPayload = JSON.stringify({
                ...
                attendeeName: updatedOrder.user?.name || updatedOrder.user?.email || "Attedee",
                status: "ISSUED",
            });
            ticketsData.push({
                id: ticketId,
 ...
 * @param {string} identifier - ticketId, ticketCode, or orderId
 * @returns {Promise<Object>} Checked-in ticket and order details
 */
export const checkInOrder = async (staffUserId, staffRole, identifier) => {
